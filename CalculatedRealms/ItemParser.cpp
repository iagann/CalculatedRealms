...
void ItemParser::ApplyStat(Stats& stats, const std::string& statName, double value = 1) {
    ...
    static std::map<std::string, std::function<void(Stats& stats, double)>> statActions = {
        ...
        {"FIRE ABILITIES WILL ALSO BENEFIT FROM", [&](Stats& stats, double value) { stats.damage.elemental.mainType = DamageElemental::ELEMENT_TYPE_FIRE; stats.damage.elemental.secondaryType = DamageElemental::ELEMENT_TYPE_LIGHTNING; }},
...
std::vector<Stats> ItemParser::ParseStatsFromFile(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        static std::vector<std::string> ignoreStats = {
            ...
            // dragon
            "extremely powerful yet friendly", "powerful yet friendly",
            "WHER ENEMIES DIiE THEIR CORPSE HAS A CHANCE TO", "EXPLODE DEALING FiRE DAMAGE",
            "WHERN ENEMIES DIiE THEIR CORPSE HAS A", "CHANCE To EXPLODE DEALING FiRE",
            ...
            // fossil
            "An ancient life source with a powerful force f", "An ancient life source with a", "powerful force Can be imbued with",
            "magical gems You only need ONE", 
...
Stats ItemParser::ParseCardsFromFile(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        int cardCount;
        std::istringstream iss(line.substr(0, openParen));
        if (!(iss >> cardCount)) {
            Util::ResetConsoleColor();
