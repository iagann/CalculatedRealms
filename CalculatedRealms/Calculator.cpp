...
std::pair<double, double> Calculator::getRating(const std::map<std::string, int>&combo) {
	...
	// damage rating
	{
		...
		intermediate = {
					...
					total.per.critDamagePerArmour * armour,
		};
		...
		if (verbose) {
			...
			if (intermediate[5]) std::cout << "\t      + Crit damage from armour enchant: " << intermediate[5] * 100 << "%" << std::endl;
		...
		damageRating = multiplied;
	}

	return { damageRating, survivabilityRating };
}
